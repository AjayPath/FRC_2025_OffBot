// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands;

import edu.wpi.first.math.geometry.Rotation2d;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.utils.Calculations;
import frc.robot.utils.Pose;
import frc.robot.utils.SimPID;
import frc.robot.utils.Vector;

/* You should consider using the more terse Command factories API instead https://docs.wpilib.org/en/stable/docs/software/commandbased/organizing-command-based.html#defining-commands */
public class TestDriveToPointwPID extends Command {
  /** Creates a new TestDriveToPointCommand. */
  private SimPID pidDistance;
  private SimPID turnPID;

  private Pose currentPose;
  private Pose targetPose;

  private double distance;
  private double speedOutput;
  private double turnSpeed;

  private double angleDiff;

  public TestDriveToPointwPID() {
    // Use addRequirements() here to declare subsystem dependencies.
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    currentPose = new Pose (0, 0, Rotation2d.fromDegrees(45));
    targetPose = new Pose(10, 10, Rotation2d.fromDegrees(120));

    pidDistance = new SimPID(0.2, 0, 0, 0);
    pidDistance.setDesiredValue(0);
    pidDistance.setMaxOutput(1);

    turnPID = new SimPID(0.2, 0, 0, 0);
    turnPID.setDesiredValue(0);
    turnPID.setMaxOutput(30);

    updateDashboard();
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {

    Vector errorVector = targetPose.Subtract(currentPose);
    distance = Math.sqrt(errorVector.GetXValue() * errorVector.GetXValue() + errorVector.GetYValue() * errorVector.GetYValue());

    speedOutput = -pidDistance.calcPID(distance);

    angleDiff = Calculations.signedAngleDifference(targetPose.GetAngleValue(), currentPose.GetAngleValue());

    turnSpeed = -turnPID.calcPID(angleDiff);


    if (Math.abs(angleDiff) > 0.5) {
      double currentAngle = currentPose.GetAngleValue() + turnSpeed * 0.02;
      Rotation2d newAngle = Calculations.NormalizeAngle(
        Rotation2d.fromDegrees(currentAngle)
      );

      currentPose.SetAngle(newAngle);
    }

    if (distance > 0.01) {
      
      double unitX = errorVector.GetXValue() / distance;
      double unitY = errorVector.GetYValue() / distance;
      double moveX = unitX * speedOutput * 0.05;
      double moveY = unitY * speedOutput * 0.05;

      currentPose.SetX(currentPose.GetXValue() + moveX);
      currentPose.SetY(currentPose.GetYValue() + moveY);

    }


    updateDashboard();

  }


  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {}

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    Vector errorVector = targetPose.Subtract(currentPose);
    double distance = Math.sqrt(errorVector.GetXValue() * errorVector.GetXValue() + errorVector.GetYValue() * errorVector.GetYValue());
    return (distance <= 0.1) && (Math.abs(angleDiff) <= 0.5);
  }

  private void updateDashboard() {
    SmartDashboard.putNumber("Current x", currentPose.GetXValue());
    SmartDashboard.putNumber("Current Y", currentPose.GetYValue());
    SmartDashboard.putNumber("Current Angle", currentPose.GetAngleValue());
    SmartDashboard.putNumber("Target X", targetPose.GetXValue());
    SmartDashboard.putNumber("Target Y", targetPose.GetYValue());
    SmartDashboard.putNumber("Target Angle", targetPose.GetAngleValue());
    SmartDashboard.putNumber("Distance to Target", distance);
    SmartDashboard.putNumber("PID Speed Output", speedOutput);
    SmartDashboard.putNumber("PID Turn Ouput", turnSpeed);
  }


}